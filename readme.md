...
</code></div></div></pre><h2>Usage</h2><p>To use ExclusiveJs, simply require the package in your Node.js application and call the <code>init</code> function, Here is an example:
...
<span class="hljs-keyword">const</span> <span class="hljs-title class_">ExclusiveJs</span> = <span class="hljs-built_in">require</span>(<span class="hljs-string">'exclusivejs'</span>).instance();
...
<span class="hljs-title class_">ExclusiveJs</span>.<span class="hljs-title function_">setRoutePath</span>(<span class="hljs-string">"src/routes"</span>) <span class="hljs-comment">// set the path to your routes folder</span>
  ...
  .<span class="hljs-title function_">connectDocumentation</span>(
                     "/api-docs",swaggerUi.serve,swaggerUi.setup(swaggerDocument) //swagger document
